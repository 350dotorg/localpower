...
def show(request):
    nav_selected = "events"
    map_events = Event.objects.filter(is_private=False, geom__isnull=False).select_related("geom")
    events = Event.objects.filter(is_private=False).select_related("geom").order_by("-when", "-start")[:4]
